...
/// <summary>
/// Rate limiting test isolated in its own factory to avoid sharing rate limiter
/// state with other tests, preventing flaky results from execution order.
/// </summary>
public sealed class MessageRateLimitIntegrationTests : IClassFixture<AuthWebApplicationFactory>
{
    private readonly HttpClient _client;

    public MessageRateLimitIntegrationTests(AuthWebApplicationFactory factory)
    {
        _client = factory.CreateClient();
    }
